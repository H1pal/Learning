...
import { TodoStateContext } from './Body';

function TodoList() {
  // 1. Context에서 todo만 추출
  const { todo } = useContext(TodoStateContext);

  // 2. 성능 최적화(useMemo): todo 배열이 변경될 때만 전체/완료/미완료 개수를 연산
  ...
  // 3. 검색창에 입력된 텍스트 상태 관리
  const [searchValue, setSearchValue] = useState("");
  const onChangeSearch = (e) => { 
    setSearchValue(e.target.value)
  };

  // 4. 검색어가 비어있으면 현재 todo 자체를, 입력되어 있으면 해당 글자가 포함된 todo를 반환
    ...
    <div id='todoList_container'>
      ...
      <div id="todo-count_container">
        <div className='count_content'>
          <p><strong>전체: {totalCount}</strong></p>
        </div>
        <div className='count_content'>
          <p><strong>완료된 할 일: {doneCount}</strong></p>
        </div>
        <div className='count_content'>
          <p><strong>미완료된 할 일: {notDoneCount}</strong></p>
        </div>
      </div>
